...
path = r"E:\GZ_chargeStation\广州市EV数据\1"
...
for m in range(len(hj)):
    ...
    lat = hj['latitude'].loc[m]
    long = hj['longitude'].loc[m]
    ...
    for n in range(len(text1)):
        ...
        data['latitude'] = hj['latitude']
        data['longitude'] = hj['longitude']
...
tj2 = pd.pivot_table(result2, index=['充电站id', '功率','latitude','longitude'], columns=['状态'], aggfunc='count')
tj2 = tj2.reset_index()
print(tj2)
tj2.to_csv('result_35.csv', index=False, encoding= 'utf-8-sig')
# 根据需求重命名列名
# columns = []
# ls = len(tj2.columns)
# for i in range(ls):
